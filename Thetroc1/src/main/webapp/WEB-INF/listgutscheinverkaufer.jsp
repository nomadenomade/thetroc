...
<html lang="en">
  ...
  <body  style="background-image: url('<%= request.getContextPath()%>/background1.jpg');background-size:cover;background-repeat:no-repeat;background-attachment:fixed;">
		...
		<nav class="navbar navbar-expand-lg navbar-light" style="background-color:#48D1CC;">
		  ...
		  <div class="container-fluid">
		    ...
		    <div class="collapse navbar-collapse" id="navbarSupportedContent">
		      <!-- Navbar brand -->
		      <a class="navbar-brand mt-2 mt-lg-0"  href="<%= request.getContextPath()%>/RequestReSendSeller?name=verkaufer_home">
		        <img
		          src="img/logo3.png"
			      height="80"
		          alt=""
		    ...
		    <div class="d-flex align-items-right">
		      ...
		      <!-- Anmeldung -->
		      <a   class="text-reset me-3"  href="<%= request.getContextPath()%>/RequestReSendSeller?name=verkaufer_home" >
    ...
    //Sessionhandling
    var idverkaufer = <%=request.getSession().getAttribute("idverkaufer") %>;
    if(idverkaufer==null){
    ...
    setInterval(function(){
    	idverkaufer = <%=request.getSession().getAttribute("idverkaufer") %>;
    	if(idverkaufer==null){	
    		 var XmL = initRequest();
   		  XmL.onreadystatechange = function(){
   			  if(this.readyState==4 && this.status==200){
  ...
  // Load Bills
  		 var XmL = initRequest();
    		  ...
    		  XmL.open("GET","Bestellung_verwaltung_verkaufer?type=quittungladen",false);
    		  XmL.send();
